...
class SeedMarketFragment : Fragment() {
    ...
    // 씨앗상점 업데이트 함수
    private fun updateSeedMarketUpdateSpfDB() {
        val mSharedPreferences: SharedPreferences = requireActivity().getSharedPreferences("SeedMarket", Context.MODE_PRIVATE)
        val editor: SharedPreferences.Editor = mSharedPreferences.edit()

        Log.e("SeedMarket", mSharedPreferences.getInt("marketVersion", 0).toString())
        if (mSharedPreferences.getInt("marketVersion", 0) != 1) {
            // 아이템 가격 변경
            try {
                ...
                Log.d("SeedMarket :: 오류", "이미 있는 행입니다.")
                Log.d("SeedMarket :: 오류", e.printStackTrace().toString())
